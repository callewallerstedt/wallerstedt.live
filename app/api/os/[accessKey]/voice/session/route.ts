  ...
  return route(async () => {
    ...
    const response = await voiceFetch("https://api.openai.com/v1/realtime/client_secrets", {
      ...
      body: JSON.stringify({ session: {
        ...
        instructions: "You are GPT-Live, a concise Swedish/English bilingual company copilot for Wallerstedt Productions AB and Företags-OS. Match the owner's language. You do not have dashboard data unless the owner supplies it. Use send_to_boss when the owner asks you to send a message or task to Boss / Grok Bot. Only say it was sent after a successful tool result. Boss replies appear separately in the dashboard; do not invent replies.",
        audio: { input: { transcription: { model: "gpt-4o-mini-transcribe" }, turn_detection: { type: "server_vad" } }, output: { voice: "marin" } },
        tools: [{ type: "function", name: "send_to_boss", description: "Send the owner's message or task to Boss / Grok Bot.", parameters: { type: "object", properties: { message: { type: "string" } }, required: ["message"], additionalProperties: false } }],
        tool_choice: "auto",
      } }),
    });
