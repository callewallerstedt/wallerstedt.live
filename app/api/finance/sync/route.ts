...
import { syncFinance } from "@/lib/finance/store";
...
/** Vercel cron, four times a day: PSD2 allows four unattended reads per account. */
export async function GET(request: Request) {
  return route(async () => {
    requireCron(request);
    return privateJson(await syncFinance({ force: true }));
  });
}
