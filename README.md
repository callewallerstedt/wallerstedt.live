...
The floating Live mic on every signed-in Bolag page opens fullscreen OpenAI Realtime voice with a two-sided transcript. Ask Live to message a specialist using `send_to_agent`: elon (ops), bjorn (coding), jensen (accountant), max (music), nils (school), jan (life advice), or rayner (trading). Successful delivery shows **Sent to Jensen**, for example. Configure `VOICE_AGENT_WEBHOOKS` as a server-only JSON map of agent slugs to `{ "url": "https://example.com/voice", "token": "replace-me" }`. Names are case-insensitive; `björn` aliases `bjorn` and `boss` aliases `elon`. The JSON map takes precedence; Elon falls back to `BOSS_VOICE_WEBHOOK_URL` / `BOSS_VOICE_WEBHOOK_TOKEN` when absent. `send_to_boss` and `POST /api/os/<key>/voice/boss` remain Elon aliases; `POST /api/os/<key>/voice/agent` accepts `{ "agent": "jensen", "message": "…" }`. Outbound webhooks receive `{ message, source: "wallerstedt-dash", agent, timestamp }` with Bearer authentication. Configure the server-only OpenAI and webhook variables in `.env.example` (`gpt-realtime` by default). The browser receives only a short-lived Realtime credential. Setup follows the [OpenAI Realtime WebRTC API](https://developers.openai.com/api/docs/guides/voice-webrtc).
